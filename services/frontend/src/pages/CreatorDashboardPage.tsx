...
import { getMySongs } from '../services/musicService';
import AppShell from '../components/layout/AppShell';
import type { AnalyticsStats, HeatmapDropOff, CreatorSongRow, MySong } from '../types/domain';
...
function mapMySongToCreatorRow(s: MySong): CreatorSongRow {
  return {
    songId: s.songId,
    title: s.title,
    coverUrl: s.coverUrl,
...
export default function CreatorDashboardPage() {
  ...
  // Load creator's songs
  useEffect(() => {
    if (role !== 'Creator' && role !== 'Admin') return;
    getMySongs().then((songs) => {
      setMySongs(songs);
      if (songs.length > 0) {
        setSelectedSong({ id: songs[0].songId, title: songs[0].title });
      }
    }).catch(() => {});
  }, [role]);
  ...
  useEffect(() => {
    if (selectedSong && (role === 'Creator' || role === 'Admin')) {
      loadAnalytics(selectedSong.id, timeRange);
    }
  }, [selectedSong, timeRange, role, loadAnalytics]);
      ...
      <div className="p-xl max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-2">
          <div className="flex items-center gap-4">
            ...
            {mySongs.length > 0 && (
              <div className="relative mt-1" ref={dropdownRef}>
                <button
                  ...
                  data-testid="song-selector-button"
                  ...
                  <div className="absolute top-full mt-1 left-0 bg-dark-card rounded-[8px] shadow-[rgba(0,0,0,0.5)_0px_8px_24px] border border-border-muted/20 z-50 min-w-[180px] overflow-hidden">
                    {mySongs.map((song) => (
                      <button
                        key={song.songId}
                        onClick={() => {
                          setSelectedSong({ id: song.songId, title: song.title });
                          setShowDropdown(false);
                        }}
                        className={[
                          'w-full text-left px-4 py-2.5 text-[14px] hover:bg-mid-dark transition-colors',
                          song.songId === selectedSong?.id ? 'text-spotify-green font-bold' : 'text-text-base',
                        ].join(' ')}
                      >
                        {song.title}
                      </button>
            ...
            {mySongs.length === 0 && !loading && (
              <span className="text-text-secondary text-sm">Chưa có bài hát nào</span>
            )}
          </div>
        ...
        {!loading && error && (
          <div role="alert" className="flex flex-col items-center gap-4 py-16 text-center">
            ...
            <button
              onClick={() => selectedSong && loadAnalytics(selectedSong.id, timeRange)}
