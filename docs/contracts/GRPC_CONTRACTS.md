...
7. [JWT Claim Structure](#7-jwt-claim-structure)
8. [Redis Blacklist](#8-redis-blacklist)
9. [JWKS Endpoint](#9-jwks-endpoint)
...
## 7. JWT Claim Structure

The Auth Service (`JwtTokenGenerator`) generates access tokens with the following claims. This is the authoritative list derived from `services/auth-service/src/AuthService.Infrastructure/Security/JwtTokenGenerator.cs`.

### Claims Included in Every Access Token

| Claim Key | JWT Registered Name | Source | Description |
|---|---|---|---|
| `sub` | `JwtRegisteredClaimNames.Sub` | `userId` (Guid) | Subject — the authenticated user's UUID v4. Used by all downstream services as the canonical user identifier. |
| `http://schemas.microsoft.com/ws/2008/06/identity/claims/role` | `ClaimTypes.Role` | `role` (string) | RBAC role: `"Listener"`, `"Creator"`, or `"Admin"`. The long-form claim type is generated by `ClaimTypes.Role` in .NET. When forwarding via HTTP headers, the API Gateway maps this to `X-User-Role`. |
| `jti` | `JwtRegisteredClaimNames.Jti` | `Guid.NewGuid()` | JWT ID — a unique UUID v4 generated fresh for every token issuance. Used as the identifier for the Redis blacklist (`auth:blacklist:{jti}`). |
| `exp` | `JwtRegisteredClaimNames.Exp` | `DateTime.UtcNow + expiresInMinutes` | Expiry timestamp (Unix epoch seconds). Set to `AccessTokenExpiryMinutes` (default: 15 minutes) from configuration key `Jwt:AccessTokenExpiryMinutes`. |
| `iat` | `JwtRegisteredClaimNames.Iat` | JWT library | Issued-at timestamp (Unix epoch seconds). Automatically added by `JwtSecurityTokenHandler`. |
| `nbf` | `JwtRegisteredClaimNames.Nbf` | JWT library | Not-before timestamp. Automatically set to `iat` value by `JwtSecurityTokenHandler`. |

### Claims NOT Included (by design)

| Claim | Reason absent |
|---|---|
| `email` | Not embedded in the access token — must be fetched from User Service via `GetUserProfile` gRPC call or `GET /users/me` REST endpoint. Avoids stale email in long-lived tokens. |
| `display_name` | Same rationale as `email` — fetched dynamically, not embedded. |
| `iss` (issuer) | Not explicitly set in `JwtTokenGenerator` v1. The JWT library omits this claim when not configured. If needed for multi-tenant validation, add `Issuer` to `JwtSecurityToken` constructor and update `TokenValidationParameters.ValidIssuer`. |

### Token Expiry Configuration

| Config Key | Environment Variable Override | Default | Notes |
|---|---|---|---|
| `Jwt:AccessTokenExpiryMinutes` | `Jwt__AccessTokenExpiryMinutes` (Docker) | `15` | Access token TTL in minutes. Keep short — refresh token rotation handles session continuity. |
| `Jwt:RefreshTokenExpiryDays` | `Jwt__RefreshTokenExpiryDays` (Docker) | `7` | Refresh token TTL in days. Stored hashed in Redis (`auth:refresh:{userId}`) and in PostgreSQL. |

### Algorithm

`HS256` (HMAC-SHA256) with the symmetric secret from `Jwt:SecretKey` / `JWT_SECRET`. The API Gateway's `TokenValidationParameters` must specify `ValidAlgorithms = new[] { "HS256" }` — never allow `"none"`.

---

## 8. Redis Blacklist

When a token is revoked (via logout, token reuse detection, or password change), the Auth Service writes to the Redis blacklist. The API Gateway checks this on every `ValidateToken` call.
...
|---|---|
| Redis type | String |
| Value stored | `"1"` (presence is the signal — value is irrelevant) |
| TTL | `(token_exp - now)` seconds — **remaining lifetime of the token** |
| Write command | `SET auth:blacklist:{jti} 1 EX {remaining_seconds} NX` |
| Check command | `EXISTS auth:blacklist:{jti}` |
| Owner service | Auth Service (sole writer) |
| Reader service | API Gateway (reads on every `ValidateToken` call) |

### When the Key is Written

| Trigger | Action |
|---|---|
| `POST /api/v1/auth/logout` | Auth Service adds `auth:blacklist:{jti}` for the access token's `jti` claim. Simultaneously deletes `auth:refresh:{userId}`. |
| Refresh Token reuse detected (`TOKEN_REUSED`) | Auth Service adds `auth:blacklist:{jti}` for the current token **and** revokes all active sessions for the user (nuclear option). |
| Password change (if implemented in Phase 2) | Revoke all outstanding tokens for the user by blacklisting their `jti` values. |

### TTL Rationale

The key must expire at exactly the same time the JWT itself would have expired. Storing a blacklisted-but-already-expired token wastes Redis memory with no security benefit. Setting `TTL = token_exp - now` ensures the key disappears at the natural expiry boundary.

If `token_exp - now <= 0` (token is already expired), skip the blacklist write — an expired token is already rejected by `ValidateLifetime = true`.

### Offline Mode Caveat

When the API Gateway operates in offline JWT verification mode (Auth Service circuit breaker open — see Section 4.1), it **cannot** consult the Redis blacklist. This is a deliberate trade-off: a token revoked via logout within the last ~60 seconds may be accepted during the outage window. Downstream services MUST tolerate `X-Auth-Mode: offline` and may apply additional authorization checks for sensitive operations.

### Cross-Service Access

See `.github/REDIS_KEY_DESIGN.md` — `auth:blacklist:{jti}` is listed in the Shared / Cross-Service Keys table: written by Auth Service, read by API Gateway. No other service may read or write this key.

---

## 9. JWKS Endpoint

The Auth Service exposes a JSON Web Key Set endpoint that allows the API Gateway to fetch the JWT signing key for **offline verification** when the Auth Service is unreachable (circuit breaker open).
...
|---|---|
| Path | `/internal/jwks` |
| Access | Internal only — not exposed through the API Gateway |
| Auth | None (service-to-service, internal network only) |
| Response Content-Type | `application/json` |
| Caching | API Gateway caches the response in Redis as `auth:pubkey:cache` (TTL: 1 hour). Background task refreshes every 60 seconds. |

### Response Format — JSON Web Key Set (RFC 7517)
...
> **Note:** The current implementation uses `HS256` (symmetric HMAC) rather than `RS256` (asymmetric RSA). For symmetric keys, the JWKS response exposes the raw key material — this endpoint **must never be reachable outside the internal cluster network**. If the platform migrates to `RS256`, replace `kty: "oct"` with `kty: "RSA"` and expose only the public key components (`n`, `e`).

### How the API Gateway Uses This Endpoint

1. **At startup**: Gateway fetches `GET http://auth-service/internal/jwks` and stores the key in Redis (`auth:pubkey:cache`, TTL 1 hour) and in-process memory.
2. **Background refresh**: Every 60 seconds, the Gateway re-fetches the JWKS and updates both caches. This picks up key rotation within 60 seconds.
3. **Circuit breaker open**: When `ValidateToken` gRPC calls fail (3 consecutive `UNAVAILABLE` / `DEADLINE_EXCEEDED`), the Gateway switches to offline mode and verifies JWT signatures locally using the cached key.
4. **Stale cache guard**: If the in-process cache is older than 5 minutes and the JWKS endpoint is still unreachable, the Gateway rejects all requests with `503 SERVICE_UNAVAILABLE` rather than accepting tokens with a potentially rotated key.

### Key Rotation Process

When the JWT signing key is rotated:
1. Auth Service generates a new key and updates `Jwt:SecretKey` (rolling deployment).
2. Auth Service proactively deletes `auth:pubkey:cache` from Redis and re-writes it with the new key.
3. The Gateway's background refresh picks up the new key within 60 seconds.
4. Tokens signed with the old key will fail verification after the Gateway picks up the new key. Users must re-authenticate (refresh token flow is unaffected if the refresh token is still valid).
